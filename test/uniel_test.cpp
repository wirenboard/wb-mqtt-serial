...
TEST_F(TUnielDeviceTest, TestQuery)
{
    ...
    ASSERT_EQ(TChannelValue{154}, Dev->ReadRegisterImpl(InputReg));
    ...
    ASSERT_EQ(TChannelValue{0}, Dev->ReadRegisterImpl(RelayReg));

    SerialPort->DumpWhatWasRead();
    EnqueueRelayOnQueryResponse();
    ASSERT_EQ(TChannelValue{1}, Dev->ReadRegisterImpl(RelayReg));

    SerialPort->DumpWhatWasRead();
    EnqueueThreshold0QueryResponse();
    ASSERT_EQ(TChannelValue{0x70}, Dev->ReadRegisterImpl(ThresholdReg));

    SerialPort->DumpWhatWasRead();
    EnqueueBrightnessQueryResponse();
    ASSERT_EQ(TChannelValue{66}, Dev->ReadRegisterImpl(BrightnessReg));
}
