...
protected:
    ...
    void EnqueueReadContinuousRead(uint16_t value)
    {
        ...
        Expector()->Expect(WrapPDU({0x03, 0x00, 0x72, 0x00, 0x01}), WrapPDU({0x03, 0x02, hi, lo}), __func__);
    }
    ...
    {
        Expector()->Expect(WrapPDU({0x03, 0x00, 0x72, 0x00, 0x01}), WrapPDU({0x83, exceptionCode}), __func__);
    }
    ...
    void EnqueueReadDeviceModelEx(const std::string& model)
    {
        ...
        Expector()->Expect(WrapPDU({0x03, 0x00, 0xC8, 0x00, 0x14}), WrapPDU(response), __func__);
    }

    // Enqueue modbus exception for device_model_ex read (20 regs)
    void EnqueueReadDeviceModelExException(uint8_t exceptionCode)
    {
        Expector()->Expect(WrapPDU({0x03, 0x00, 0xC8, 0x00, 0x14}), WrapPDU({0x83, exceptionCode}), __func__);
    }
    ...
    void EnqueueReadDeviceModel(const std::string& model)
    {
        ...
        Expector()->Expect(WrapPDU({0x03, 0x00, 0xC8, 0x00, 0x06}), WrapPDU(response), __func__);
    }
...
TEST_F(TRPCDeviceLoadConfigDetailTest, ReadWbRegister_Success)
{
    ...
    auto result =
        rpc_device_load_config_detail::ReadWbRegister(*SerialPort, rpcRequest, WbRegisters::DEVICE_MODEL_REGISTER_NAME);
...
TEST_F(TRPCDeviceLoadConfigDetailTest, ReadWbRegister_ModbusExceptionThrows)
{
    ...
    EXPECT_THROW(rpc_device_load_config_detail::ReadWbRegister(*SerialPort,
                                                               rpcRequest,
