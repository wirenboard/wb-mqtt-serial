...
namespace
{
    std::unique_ptr<Modbus::IModbusTraits> MakeTraits(const std::string& protocol)
    {
...
// ============================================================
//                     TFwGetInfoTask
// Cf. firmware_update.py:224 FirmwareInfoReader class
// ============================================================
                                                   ...
                                                   const std::list<PSerialDevice>& polledDevices)
{
    ...
    try {
        ...
        auto traits = MakeTraits(Protocol);
        TFwDeviceInfo info;

        // Read firmware signature - Cf. firmware_update.py:252 read_fw_signature()
        info.FwSignature =
            ReadStringRegister(*port, *traits, FwRegisters::FW_SIGNATURE_ADDR, FwRegisters::FW_SIGNATURE_COUNT, 0x03);
        ...
        try {
            info.FwVersion =
                ReadStringRegister(*port, *traits, FwRegisters::FW_VERSION_ADDR, FwRegisters::FW_VERSION_COUNT, 0x03);
                            ...
                            presenceData))
        {
            ...
            for (int compNum: presentComponents) {
                try {
                    ...
                    std::string fwVer =
                        ReadStringRegister(*port, *traits, fwAddr, FwRegisters::COMPONENT_FW_VERSION_COUNT, 0x04);
                    std::string model =
                        ReadStringRegister(*port, *traits, modelAddr, FwRegisters::COMPONENT_MODEL_COUNT, 0x04);
...
void TFwFlashTask::DoReboot(TPort& port, Modbus::IModbusTraits& traits)
{
...
void TFwFlashTask::WriteInfoBlock(TPort& port, Modbus::IModbusTraits& traits)
{
...
void TFwFlashTask::WriteDataBlock(TPort& port, Modbus::IModbusTraits& traits, const std::vector<uint8_t>& chunk)
{
...
void TFwFlashTask::WriteDataBlocks(TPort& port, Modbus::IModbusTraits& traits)
{
    const size_t CHUNK_SIZE = FwRegisters::FW_DATA_BLOCK_COUNT * 2; // 68 registers * 2 bytes = 136 bytes

    size_t totalChunks = (Firmware.Data.size() + CHUNK_SIZE - 1) / CHUNK_SIZE;
    if (totalChunks == 0 && Firmware.Data.empty()) {
        if (OnProgress) {
            OnProgress(100);
        }
        return;
    ...
    for (size_t i = 0; i < totalChunks; ++i) {
        ...
        size_t chunkLen = std::min(CHUNK_SIZE, Firmware.Data.size() - offset);
        std::vector<uint8_t> chunk(Firmware.Data.begin() + offset, Firmware.Data.begin() + offset + chunkLen);
        ...
        WriteDataBlock(port, traits, chunk);
                                                 ...
                                                 const std::list<PSerialDevice>& polledDevices)
{
    try {
        ...
        if (RebootToBootloader) {
            DoReboot(*port, *traits);
        }

        WriteInfoBlock(*port, *traits);
        WriteDataBlocks(*port, *traits);
