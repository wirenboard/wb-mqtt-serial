...
namespace
{
    ...
    PPort OpenSerialPort(const Json::Value& port_data, PRPCHandler rpcHandler)
    {
        ...
        rpcHandler->AddPort(port, TRPCModbusMode::RPC_RTU, settings.Device, "", 0);

        return port;
    }

    PPort OpenTcpPort(const Json::Value& port_data, PRPCHandler rpcHandler)
    {
        TTcpPortSettings settings(port_data["address"].asString(), GetInt(port_data, "port"));

        PPort port = std::make_shared<TTcpPort>(settings);

        rpcHandler->AddPort(port, TRPCModbusMode::RPC_TCP, "", settings.Address, settings.Port);
                  ...
                  TPortFactoryFn portFactory)
    {
        ...
        std::tie(port_config->Port, port_config->IsModbusTcp) = portFactory(port_data, rpcHandler);
...
std::pair<PPort, bool> DefaultPortFactory(const Json::Value& port_data, PRPCHandler rpcHandler)
{
    auto port_type = port_data.get("port_type", "serial").asString();
    if (port_type == "serial") {
        return {OpenSerialPort(port_data, rpcHandler), false};
    }
    if (port_type == "tcp") {
        return {OpenTcpPort(port_data, rpcHandler), false};
    }
    if (port_type == "modbus tcp") {
        return {OpenTcpPort(port_data, rpcHandler), true};
    }
