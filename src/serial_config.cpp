...
bool isCustomisableRegister(const Json::Value& registerSchema)
{
    return (registerSchema.isMember("oneOf") || registerSchema.isMember("device_type"));
}
...
Json::Value MakeSetupRegisterSchema(const Json::Value& setupRegister, int index)
{
    ...
    if (setupRegister.isMember("default")) {
        r["default"] = setupRegister["default"];
    }
    if (setupRegister.isMember("enum")) {
        r["enum"] = setupRegister["enum"];
    }
    if (setupRegister.isMember("enum_titles")) {
        r["options"]["enumTitles"] = setupRegister["enum_titles"];
    }
...
Json::Value MakeTabSingleDeviceChannelSchema(const Json::Value& channel, const std::string& deviceDefinitionPrefix)
{
    Json::Value r;
    r["headerTemplate"] = channel["name"].asString();
    r["$ref"] = "#/definitions/" + deviceDefinitionPrefix + GetHashedParam(channel["device_type"].asString(), "d");
    return r;
}
                               ...
                               bool allowCollapse = false)
{
    ...
    if (format == "default") {
        for (const auto& channel: channels) {
            if (isCustomisableRegister(channel)) {
                tabs = true;
                break;
            }
        }
...
void MakeDevicesForConfed(Json::Value& devices, ITemplateMap& templates, size_t nestingLevel)
{
    for (Json::Value& device : devices) {
        if (device.isMember("device_type")) {
            device = MakeDeviceForConfed(device, templates, nestingLevel + 1);
        }
    }
}
    ...
    public:
        TSubDevicesTemplateMap(const Json::Value& device)
        {
            if (device.isMember("subdevices")) {
                for (auto& device: device["subdevices"]) {
                    Templates.insert({device["device_type"].asString(), device["device"]});
                }
            }
        }
...
Json::Value MakeDeviceForConfed(const Json::Value& config, ITemplateMap& deviceTemplates, size_t nestingLevel)
{
    ...
    AppendParams(newDev, setupRegs.second);
    ...
    if ( standardChannels.size() ) {
        if (nestingLevel == 1) {
            TSubDevicesTemplateMap templates(config);
...
Json::Value MakeSubDeviceUISchema(const std::string& deviceType, const Json::Value& subdeviceShema, const std::string& devicePrefix)
{
    ...
    auto set = GetHashedParam(deviceType, "s") + "_e";
    ...
    if (subdeviceShema.isMember("ui_options")) {
        if (subdeviceShema["ui_options"].isMember("setup_format")) {
            res["properties"][set]["_format"] = subdeviceShema["ui_options"]["setup_format"];
        }
    ...
    if (subdeviceShema.isMember("setup")) {
        for (const auto& setupRegister: subdeviceShema["setup"]) {
            if (!setupRegister.isMember("value")) {
                res["properties"][set]["properties"]["set" + to_string(i)] = MakeSetupRegisterSchema(setupRegister, i);
                ++i;
            }
        }
    }
...
std::pair<Json::Value, Json::Value> MakeDeviceUISchema(const std::string& deviceType, const Json::Value& t)
{
    ...
    if (t.isMember("ui_options")) {
        if (t["ui_options"].isMember("setup_format")) {
            res["properties"][set]["_format"] = t["ui_options"]["setup_format"];
        }
    ...
    if (t.isMember("setup")) {
        ...
        for (const auto& setupRegister: t["setup"]) {
            if (!setupRegister.isMember("value")) {
                res["properties"][set]["properties"]["set" + to_string(i)] = MakeSetupRegisterSchema(setupRegister, 8);
                ++i;
            }
        }
    }
    ...
    if (t.isMember("subdevices")) {
        for (const auto& subDevice: t["subdevices"]) {
            ...
            definitions[set + GetHashedParam(name, "d")] = MakeSubDeviceUISchema(name, subDevice["device"], set);
        }
    }
