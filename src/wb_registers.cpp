...
namespace
{
    ...
    const auto LOCAL_TIME_REGISTER_ADDRESS = 452;
    ...
    const std::unordered_map<std::string, PRegisterConfig> RegConfigs = {
        ...
        {WbRegisters::SN_REGISTER_NAME,
         TRegisterConfig::Create(Modbus::REG_HOLDING,
                                 ...
                                 U32)},
        {WbRegisters::LOCAL_TIME_REGISTER_NAME,
         TRegisterConfig::Create(
             Modbus::REG_HOLDING,
             TRegisterDesc{std::make_shared<TUint32RegisterAddress>(LOCAL_TIME_REGISTER_ADDRESS), 0, 0},
             U64)},
    };
}
