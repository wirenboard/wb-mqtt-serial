...
namespace // utility
{
    ...
    template<>
    inline std::string ToScaledTextValue(const TBasicVirtualRegister & reg, float val)
    {
        return StringFormat("%.7g", RoundValue(reg.Scale * val + reg.Offset, reg.RoundTo));
    }

    template<>
    inline std::string ToScaledTextValue(const TBasicVirtualRegister & reg, double val)
    {
        return StringFormat("%.15g", RoundValue(reg.Scale * val + reg.Offset, reg.RoundTo));
    }

    template<typename A>
    inline std::string ToScaledTextValue(const TBasicVirtualRegister & reg, A val)
    {
    ...
    template<typename T>
    T FromScaledTextValue(const TBasicVirtualRegister & reg, const std::string& str);

    template<>
    inline uint64_t FromScaledTextValue(const TBasicVirtualRegister & reg, const std::string& str)
    {
    ...
    template<>
    inline int64_t FromScaledTextValue(const TBasicVirtualRegister & reg, const std::string& str)
    {
        if (reg.Scale == 1 && reg.Offset == 0) {
    ...
    template<>
    inline double FromScaledTextValue(const TBasicVirtualRegister & reg, const std::string& str)
    {
        return (RoundValue(stod(str), reg.RoundTo) - reg.Offset) / reg.Scale;
    }

    uint64_t InvertWordOrderIfNeeded(const TBasicVirtualRegister & reg, const uint64_t value)
    {
    ...
    string ConvertSlaveValue(const TBasicVirtualRegister & reg, uint64_t value)
    {
        switch (reg.Format) {
        case S8:
    ...
    uint64_t ConvertMasterValue(const TBasicVirtualRegister & reg, const std::string& str)
    {
        switch (reg.Format) {
        case S8:
...
TBasicVirtualRegister::TBasicVirtualRegister(const PRegisterConfig & config, const PSerialDevice & device, PBinarySemaphore flushNeeded, TInitContext & context)
    ...
    , ErrorState(EErrorState::UnknownErrorState)
{
    ...
    WriteQuerySet = make_shared<TIRDeviceQuerySet>(GetKeysAsSet(ProtocolRegisters), EQueryOperation::WRITE);
}

uint32_t TBasicVirtualRegister::GetBitPosition() const
{
    return (uint64_t(Address) * ProtocolRegisterWidth * 8) + BitOffset;
}

uint32_t TBasicVirtualRegister::GetBitSize() const
{
    return ProtocolRegisterWidth * 8 * ProtocolRegisters.size();
}

void TBasicVirtualRegister::AssociateWith(const PProtocolRegister & reg, const TRegisterBindInfo & bindInfo)
{
...
bool TBasicVirtualRegister::UpdateReadError(bool error)
{
...
bool TBasicVirtualRegister::UpdateWriteError(bool error)
{
    EErrorState newState;
    if (error) {
...
bool TBasicVirtualRegister::AcceptDeviceValue(bool & changed)
{
...
bool TBasicVirtualRegister::AcceptDeviceReadError()
{
    return UpdateReadError(true);
}

size_t TBasicVirtualRegister::GetHash() const
{
...
bool TBasicVirtualRegister::operator==(const TVirtualRegister & rhs) const noexcept
{
    if (this == &rhs) {
        return true;
    }

    auto pRhs = dynamic_cast<const TBasicVirtualRegister *>(&rhs);
...
const PSerialDevice & TBasicVirtualRegister::GetDevice() const
{
    return Device;
}

const TPSet<TProtocolRegister> & TBasicVirtualRegister::GetProtocolRegisters() const
{
    return GetKeysAsSet(ProtocolRegisters);
}

bool TBasicVirtualRegister::NeedToPublish() const
{
...
EErrorState TBasicVirtualRegister::GetErrorState() const
{
    return ErrorState;
}

uint64_t TBasicVirtualRegister::GetValue() const
{
    return MapValueFrom(ProtocolRegisters);
}

std::string TBasicVirtualRegister::GetTextValue() const
{
    return ConvertSlaveValue(*this, InvertWordOrderIfNeeded(*this, GetValue()));
}

const std::string & TBasicVirtualRegister::GetTypeName() const
{
    return TypeName;
}

void TBasicVirtualRegister::SetTextValue(const std::string & value)
{
    Dirty.store(true);
...
bool TBasicVirtualRegister::NeedToFlush() const
{
    return Dirty.load();
}

bool TBasicVirtualRegister::Flush()
{
    if (Dirty.load()) {
        Dirty.store(false);
...
void TBasicVirtualRegister::NotifyPublished()
{
...
bool TBasicVirtualRegister::NotifyRead(const PProtocolRegister & reg, bool ok)
{
