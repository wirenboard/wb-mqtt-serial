...
4. В файле нового шаблона, в `device_type` добавить префикс с версией `tpl1_`, `tpl2_` и т.д.).
   Например:
   - Из старого типа `"device_type": "danfoss_ekc_204a1_wb_ref",`
   - В новом файле получится такой `"device_type": "tpl1_danfoss_ekc_204a1_wb_ref",`

   Обратите внимание - что тип у старого устаревшего шаблона менять нельзя - чтобы он остался рабочим у тех кто его уже выбрал.
