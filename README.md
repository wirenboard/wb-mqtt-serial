...
При старте по-умолчанию драйвер завершается с ошибкой. Добавьте устройства в `/etc/wb-homa-modbus.conf` либо воспользуйтесь онлайн-редактором настроек для начала работы.
    ...
    "ports": [
        {
            ...
            // Задержка после получения ответа по RS-485 в миллисекундах
            // драйвер будет ожидать <poll_interval> миллисекунд после каждой команды
            // вся активность на данном порту будет прекращена в период ожидания,
            //   включая операции записи
...
Для работы устройств необходимо выставить протокол "uniel" для порта, работа с устройствами Modbus на одном порте не поддерживается.
