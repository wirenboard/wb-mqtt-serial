...
В Modbus устройствах Wiren Board есть регистры, которые показывают число нажатий (простых, длинных, двойных и т.п.). Рекомендуется использовать их вместо прямого чтения состояния входов для организации реакции на нажатия во внешних системах. Обычно это 16-битные регистры, значение которых монотонно увеличивается. При достижении максимального значения отсчёт начинается заново с 0. При перезагрузке устройства значение сбрасывается в 0.
...
Сервис записывает состояния регистров во внутреннюю базу и при старте публикует эти сохранённые значения. Сохранённые значения сбрасываются на диск раз в минуту. Если значения не успели записать на диск, при запуске сервиса будет опубликовано какое-то старое значение, а потом текущее, полученное чтением из регистра. При записи значений регистров с типом `press_counter` база сразу сбрасывается на диск, чтобы избежать публикации неактуальных значений.
