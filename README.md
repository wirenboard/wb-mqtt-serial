...
Шаблоны конфигурации создаются для удобства конфигурации Modbus-устройств.
...
В контроллер Wiren Board изначально загружены шаблоны кофигурации некоторых поддерживаемых устройств. Они хранятся в папке `/usr/share/wb-mqtt-serial/templates`. Для загрузки пользовательских шаблонов используется папка `/etc/wb-mqtt-serial.conf.d/templates`. 

Для создания собственного шаблона можно скопировать существующий шаблон из папки `/usr/share/wb-mqtt-serial/templates`, изменить в нем нужные параметры, добавить необходимые регистры устройства и сохранить его в папку `/etc/wb-mqtt-serial.conf.d/templates` с новым именем. 
...
Переводы используются в веб-конфигураторе для формирования интерфейса настройки устройств. Они описываются в шаблоне устройства в секции `translations`. Эта секция содержит переводы параметров устройства. Секция содержит набор пар `"Название параметра": "Перевод"`.
Например:
```jsonc
{
    "device_type": "Example",
