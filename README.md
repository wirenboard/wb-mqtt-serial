...
Существует возможность выполнить запись и чтение из порта посредством MQTT RPC запроса. Выполнение запроса встраивается в цикл опроса устройств таким образом, что запрос выполняется перед каждым циклом опроса устройств wb-mqtt-serial непосредственно, либо в течение времени ожидания следующего цикла опроса wb-mqtt-serial. 
