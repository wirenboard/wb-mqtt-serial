...
Существует возможность выполнить запись и чтение из порта посредством MQTT RPC запроса. Выполнение запроса встраивается в цикл опроса устройств таким образом, что запрос выполняется перед каждым циклом непосредственно, либо в течение времени ожидания следующего цикла. 
...
|`response_timeout`          |500 ms | таймаут чтения первого байта
|`frame_timeout`             |20 ms | таймаут чтения каждого последующего байта
|`total_timeout`             |10 s  | таймаут выполнения RPC-запроса
