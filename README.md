...
Содержимое шаблона проходит полную проверку — ту же, что выполняется при первом использовании шаблона: синтаксис JSON, наличие непустого `device_type`, соответствие JSON-схеме шаблонов, разбор `condition`-выражений, согласованность адресов параметров, совпадение полей чтения у объявлений параметров, используемых в условиях, и вложенность subdevices. Шаблоны, помеченные `"deprecated": true`, загрузить нельзя: при загрузке с диска они освобождены от этих проверок, поэтому их приём через RPC не поддерживается. Невалидный шаблон отклоняется с ошибкой, файловая система не изменяется.
