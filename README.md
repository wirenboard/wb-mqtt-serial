...
Актуальные настройки Modbus устройств Wiren Board могут быть считаны посредством MQTT RPC запроса. Выполнение запроса встраивается в цикл опроса устройств таким образом, что запрос выполнится с высоким приоритетом сразу после окончания текущего цикла опроса.
...
|`paramteters`|JSON-объект c актуальными настройками устройства (может быть пустым в случае, если никакие настройки не описаны шаблоне или не были считаны, например, из-за несоответствия версии устройства и описания настроек в шаблоне)|
