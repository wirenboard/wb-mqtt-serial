...
Типы устройств, чей действующий шаблон загружен из папки пользовательских шаблонов, драйвер помечает в ответе RPC-запроса `config/Load` опциональным полем `"user-defined": true` (у типов со штатными шаблонами поле не передаётся, его отсутствие трактуется как `false`) — веб-интерфейс показывает такие типы с отметкой «пользовательский». Начиная с версии 2.260.0 пользовательскими шаблонами можно управлять через MQTT RPC: см. разделы «[Загрузка пользовательского шаблона](#загрузка-пользовательского-шаблона)» и «[Удаление пользовательского шаблона](#удаление-пользовательского-шаблона)».
...
У параметра, который упомянут в условии `condition`, может быть несколько объявлений с одним `id`. Например, когда список значений параметра зависит от режима устройства. Все такие объявления должны читать регистр одинаково. Поля `reg_type`, `format`, `scale`, `offset`, `round_to`, `word_order` и `byte_order` обязаны совпадать, и сравниваются они в том виде, как записаны в шаблоне. Остальные поля, в том числе `enum`, могут различаться. Начиная с версии 2.265.0 драйвер проверяет это при первом использовании шаблона и при загрузке шаблона через `templates/Upload`.
...
Начиная с версии 2.260.0 можно загрузить файл шаблона в папку пользовательских шаблонов (`/etc/wb-mqtt-serial.conf.d/templates`) посредством MQTT RPC запроса.
Для выполнения запроса необходимо отправить в топик `/rpc/v1/wb-mqtt-serial/templates/Upload/client_id`, где `client_id` - произвольное имя клиента, посылающего запрос, сообщение типа JSON со следующими параметрами:

#### Параметры

|Параметр  |Тип          |Значение по умолчанию |Описание |
|----------|-------------|----------------------|---------|
|`content` |обязательный |                      |полный текст файла шаблона (JSON)|
|`filename`|обязательный |                      |имя файла для сохранения шаблона. Сервис берёт имя файла без пути, заменяет символы вне `[A-Za-z0-9._-]` на `_` и добавляет расширение `.json`, если его нет; запись за пределы папки пользовательских шаблонов невозможна|
|`lang`    |опциональный |`en`                  |язык локализованных полей ответа (как у `config/Load`)|
|`force`   |опциональный |`false`               |подтверждение перекрытия шаблона типа, используемого устройствами текущей конфигурации|

Содержимое шаблона проходит полную проверку — ту же, что выполняется при первом использовании шаблона: синтаксис JSON, наличие непустого `device_type`, соответствие JSON-схеме шаблонов, разбор `condition`-выражений, согласованность адресов параметров, совпадение полей чтения у объявлений параметров, используемых в условиях, согласованность объявлений одного параметра с разными `fw` и вложенность subdevices. Шаблоны, помеченные `"deprecated": true`, загрузить нельзя: при загрузке с диска они освобождены от этих проверок, поэтому их приём через RPC не поддерживается. Невалидный шаблон отклоняется с ошибкой, файловая система не изменяется.
...
Начиная с версии 2.260.0 можно удалить пользовательский шаблон по типу устройства посредством MQTT RPC запроса. Штатные шаблоны (`/usr/share/wb-mqtt-serial/templates`) удалить через RPC нельзя.
Для выполнения запроса необходимо отправить в топик `/rpc/v1/wb-mqtt-serial/templates/Delete/client_id`, где `client_id` - произвольное имя клиента, посылающего запрос, сообщение типа JSON со следующими параметрами:

#### Параметры

|Параметр|Тип          |Значение по умолчанию |Описание |
|--------|-------------|----------------------|---------|
|`type`  |обязательный |                      |тип устройства (`device_type`), пользовательский шаблон которого нужно удалить|
|`lang`  |опциональный |`en`                  |язык локализованных полей ответа (как у `templates/Upload`)|
|`force` |опциональный |`false`               |подтверждение удаления шаблона типа, используемого устройствами текущей конфигурации (симметрично `templates/Upload`)|
