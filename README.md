    ...
    "device": {
        ...
        "groups": [
            // Описание группы
            {
                ...
                // Группа может быть объявлена несколько раз с одинаковым "id" и разными "fw"
                // (fw-варианты группы): так прошивка, добавившая параметр или значение параметра,
                // приносит своё описание группы.
                // Действует объявление с наибольшей "fw" среди поддержанных прошивкой устройства;
                // оно дополняет базовое объявление (то, у которого "fw" наименьшая), поэтому
                // задавать в нём нужно только изменяемые поля. Поля "id", "group" и "order"
                // всегда берутся из базового объявления: дерево групп строится один раз, до
                // чтения прошивки. Базовое объявление действует и тогда, когда прошивка не
                // поддерживает ни одного объявления или ещё не прочитана.
