...
- [Особенности работы драйвера](#особенности-работы-драйвера)
  ...
  - [Синхронизация времени устройства](#синхронизация-времени-устройства)
...
Некоторые устройства Wiren Board хранят локальное время в отдельном регистре. Чтобы драйвер синхронизировал это время с часами контроллера, надо задать параметр `wb_time_sync_interval` в шаблоне или настройках устройства — интервал синхронизации в часах. Ноль или отсутствие параметра отключают синхронизацию.

Первая запись выполняется при подключении устройства, следующие — не чаще заданного интервала. Если запись завершилась ошибкой, например устройство не поддерживает регистр времени, новые попытки не выполняются до переподключения устройства.
