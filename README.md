...
Тип мотора(`node type`) можно задать через параметр устройства `node_type`. По умолчанию исполльзуется Sonesse 30.
Для организации отправки команд можно использовать канал с типом регистра `command`. В этом случае заголовок пакета (`MSG`) задаётся в адресе регистра. Число, записываемое в такой регистр должны содержать длину в младшем байте. Например, `0x03020103` будетозначать что в команду будет добавлено 3 байта: 0x01, 0x02, 0x03.
Для организации запроса статуса можно использовать канал с типом регистра `param`. В этом случае заголовок запроса кодируется в младшем байте адреса регистра, а заголовок ответа - в старшем. Поддерживается возможность указать смещение и длину данных в ответе.
...
|                     Device                     | device_type    | id_prefix      | name_prefix           |
|:----------------------------------------------:|:--------------:|:--------------:|:---------------------:|
| Мотор управления шторами Akko/Dooya            | AM82Smart      | am82smart      |  AM82Smart              |
