...
Требования к именованию каналов и параметров, стилю текстов и структуре шаблонов собраны в стандартах экосистемы Wiren Board — [wb-standards](https://github.com/wirenboard/wb-standards); для шаблонов в этом репозитории они обязательны. Пошаговое руководство по созданию шаблона — [на вики](https://wiki.wirenboard.com/wiki/Connecting_Third_Party_Devices_to_Wiren_Board).
...
```jsonc
{
    ...
    "device": {
        ...
        "parameters": [
            {
                ...
                // Название параметра в веб-конфигураторе; служит ключом перевода
                // (см. раздел «Перевод названий параметров устройств»).
                // В новых шаблонах title задают равным id, а отображаемые тексты
                // выносят в переводы (WB-STD-003 §4.3)
                "title": "s22",
        ...
        "channels": [
            ...
            {
                // Имя канала — его идентификатор: из него формируется MQTT-топик
                // /devices/<id устройства>/controls/<name>, оно сохраняется в конфигурационном
                // файле пользователя и служит ключом перевода (см. раздел «Перевод названий параметров устройств»).
                // Менять name у опубликованного шаблона нельзя — у пользователей сломаются правила
                // и интеграции; переименовывайте канал через переводы.
                // Правила именования и канонические имена — WB-STD-001 (https://github.com/wirenboard/wb-standards)
                "name": "Temperature",
                "reg_type": "input",
                "format": "s32",
                "address": "0x0504",
                ...
                // Необязательный идентификатор канала: если задан, MQTT-топик формируется из него,
                // а name используется только для отображения и как ключ перевода.
                // В новых шаблонах не используется — оставлен для совместимости со старыми
                // шаблонами (WB-STD-003 §7.1)
                "id": "temperature",
        ...
        "groups": [
            // Описание группы
            {
                // Название группы в веб-конфигураторе; служит ключом перевода
                // (см. раздел «Перевод названий параметров устройств»).
                // В новых шаблонах title задают равным id (WB-STD-003 §4.3)
                "title": "Group 1",
...
Переводы используются в веб-конфигураторе для формирования интерфейса настройки устройств. Они описываются в шаблоне устройства в секции `translations`: для каждого языка (`en`, `ru`) — словарь «ключ: отображаемая строка».

Как драйвер находит текст:

- ключом служит литеральная строка из поля сущности: у канала — `name`, у параметра и группы — `title`, у шаблона в каталоге устройств — `title` верхнего уровня;
- если в словаре языка нет записи по ключу, отображается сам ключ — поэтому в старых шаблонах, где в полях записан английский текст, секции `en` часто нет;
- `device.name` не переводится: это маркировка устройства, она публикуется в MQTT как есть.

В новых шаблонах действует модель «идентификатор-как-ключ» ([WB-STD-003 §4.3](https://github.com/wirenboard/wb-standards)): в полях `name`/`title` хранятся snake_case-идентификаторы, а отображаемые строки на обоих языках задаются в `translations`. Например:

```jsonc
{
    "device_type": "example",
    "title": "example_template_title",
    "device": {
        "name": "EXAMPLE",
        "channels": [
            {
        ...
        "translations": {
            "en": {
                "example_template_title": "Example (demo device)",
                "temperature": "Temperature",
                "timeout": "Timeout",
                "g_general": "General"
            },
            "ru": {
                "example_template_title": "Example (демонстрационное устройство)",
