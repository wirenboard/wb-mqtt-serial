...
В состав пакета изначально входят шаблоны кофигурации некоторых поддерживаемых устройств. Они хранятся в папке `/usr/share/wb-mqtt-serial/templates`. Для хранения пользовательских шаблонов используется папка `/etc/wb-mqtt-serial.conf.d/templates`. 
