                    ...
                    "channels": [
                        {
                            ...
                            // подходит для "дискретных" каналов (например, счётчики нажатий, состояния входов),
                            // события для которых генерируются при каждом изменении значения
                            "sporadic": false,

                            // использовать события расширенного modbus одновременно с опросом
                            // (если поддерживается прошивкой устройства)
                            // подходит для "аналоговых" каналов, где события генерируются только при значительном изменении значения
                            // позволяет быстро реагировать на резкие изменения и не терять значения при небольших изменениях
                            "semi-sporadic": false,
