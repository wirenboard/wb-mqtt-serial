...
```
{
    ...
    // также по умолчанию берется из шаблона с добавлением slave_id и индекса порта:
    // "deviceID" + "_" + slave_id + "_" + <port index>
    // индекс порта зависит от расположения его конфигурации в массиве в файле конфигурации
    "id" : "deviceID",
