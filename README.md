    ...
    "device": {
        ...
        "groups": [
            // Описание группы
            {
                ...
                // Минимальная версия прошивки устройства, для которой действует объявление группы.
                // Если не задана - объявление действует для любой прошивки.
                // Группа, как и параметр, может быть объявлена несколько раз с одинаковым "id"
                // и разными "fw" (fw-варианты): так прошивка, добавившая параметр или значение
                // параметра, приносит своё описание группы.
                // Действует объявление с наибольшей "fw" среди поддержанных прошивкой устройства,
                // и действует целиком, поэтому каждое объявление задаёт свои "title",
                // "description" и "ui_options". Различаться объявления могут только этими полями:
                // "id", "group" и "order" берутся из первого объявления, потому что дерево групп
                // строится один раз, до чтения прошивки. Самое старое объявление действует и
                // тогда, когда прошивка не поддерживает ни одного, и до чтения прошивки.
                // Драйвер группы не обрабатывает, вариант выбирает веб-интерфейс,
                // поэтому нужен wb-mqtt-homeui не старше 2.255.0
                "fw": "1.2.3"
            },
            ...
        ]
