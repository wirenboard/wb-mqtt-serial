...
Объект `port` описывает либо последовательный порт, либо TCP-порт.

Поля последовательного порта:

|Поле        |Тип          |Значение по умолчанию |Описание |
|------------|-------------|----------------------|---------|
|`path`      |обязательный |                      |путь к последовательному порту, например `/dev/ttyRS485-1`
...
|`stop_bits` |опциональный |2                     |количество стоп-бит

Поля TCP-порта:

|Поле        |Тип          |Описание |
|------------|-------------|---------|
|`address`   |обязательный |IP-адрес или имя хоста, например `192.168.1.10`
|`port`      |обязательный |номер порта, например `23`

Порт не обязан быть описанным в конфигурации драйвера, соединение с ним устанавливается на время выполнения запроса.

Настройки порта применяются на время выполнения запроса, после его окончания порт возвращается к настройкам из конфигурации. Значения по умолчанию не берутся из конфигурации порта, поэтому для устройства, которое работает не на 9600 8N2, фактические настройки надо передавать в запросе.
...
```jsonc
{
    ...
    // более новую прошивку или загрузчик, и конфигурация инсталляции позволяет обновить её
    "can_update": false,
