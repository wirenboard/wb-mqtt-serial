...
- [Особенности работы драйвера](#особенности-работы-драйвера)
  ...
  - [Таймауты чтения фрейма: frame_timeout_ms и force_frame_timeout](#таймауты-чтения-фрейма-frame_timeout_ms-и-force_frame_timeout)
  ...
  - [Список портов и опрашиваемых устройств](#список-портов-и-опрашиваемых-устройств)
...
- [Служебный MQTT-RPC API](docs/private_rpc.md)
- [Протоколы](#протоколы)
...
- GTD IOT (кнопочные панели серии 24A/24X),
...
```jsonc
{
    ...
    // Значение 0 — публиковать каждое прочитанное значение.
    // Отрицательное значение — публиковать только при изменении. Это поведение по умолчанию.
    // Положительные значения меньше 5 секунд автоматически поднимаются драйвером до 5 секунд.
    ...
    "ports": [
        {
            ...
            // Таймаут подключения (мс) — первый из двух критериев разрыва соединения.
            // Соединение сбрасывается и переоткрывается, когда ОДНОВРЕМЕННО выполнены оба условия:
            //   1) все устройства на порту переведены драйвером во внутреннее состояние «отключено»
            //      (см. device_timeout_ms / device_max_fail_cycles),
            //   2) с момента последнего успешного обмена прошло не менее connection_timeout_ms
            //      И подряд набрано не менее connection_max_fail_cycles неуспешных циклов.
            // Значение 0 — условие по времени всегда выполнено, разрыв
            // определяется только числом неуспешных циклов.
            // Значение -1 — механизм переподключения выключен целиком
            // (независимо от connection_max_fail_cycles).
            "connection_timeout_ms": 5000,

            // Количество неудачных циклов опроса — второй из двух критериев разрыва соединения.
            // Работает вместе с connection_timeout_ms по логике «И» (см. выше).
            // Значение 0 — условие по числу циклов всегда выполнено,
            // разрыв определяется только по времени.
            // Значение -1 — механизм переподключения выключен целиком
            // (независимо от connection_timeout_ms).
            ...
            "devices": [
                {
                    ...
                    // Интервал синхронизации времени устройства в часах.
                    // Драйвер записывает локальное время контроллера в регистр времени устройства.
                    // Первая запись выполняется при первом опросе после подключения устройства,
                    // следующие - не чаще заданного интервала.
                    // Ноль или отсутствие параметра отключают синхронизацию.
                    // Поддерживается только в устройствах Wiren Board с регистром локального времени.
                    "wb_time_sync_interval": 24,

                    // Максимальное число считываемых промежуточных регистров
                    // Для ускорения опроса драйвер может объединять чтение соседних регистров в один запрос (читать их «пачкой»).
                    // Этот параметр задаёт, сколько подряд идущих регистров, не описанных в конфигурации, допустимо включать в такую пачку, чтобы не разрывать её.
                    // Поддерживается только устройствами Modbus.
                    "max_reg_hole": 10,
                    ...
                    // Максимальное число регистров или coils/discrete inputs, считываемых за один запрос
                    // Максимальное количество 16-битных регистров либо однобитных coils/discrete inputs в одной пакетной операции чтения (в данный момент поддерживается только устройствами Modbus).
                    // Если равно нулю или превышает максимально допустимое значение, 
                    // используется максимально допустимое значение (125 для 16-битных регистров и 2000 для coils/discrete inputs).
                    ...
                    // Время ожидания устройства (мс) — первый из двух критериев отключения устройства.
                    // Драйвер переводит устройство во внутреннее состояние «отключено» (DISCONNECTED),
                    // когда ОДНОВРЕМЕННО выполнены оба условия:
                    //   1) с момента последнего успешного обмена прошло не менее device_timeout_ms,
                    //   2) подряд набрано не менее device_max_fail_cycles циклов, в каждом из которых
                    //      не был успешно прочитан ни один регистр устройства.
                    // Значение 0 — условие по времени всегда выполнено,
                    // отключение определяется только числом неуспешных циклов.
                    // Значение -1 — механизм отключения устройства выключен целиком
                    // (независимо от device_max_fail_cycles). Устройство всегда
                    // считается подключённым.
                    "device_timeout_ms": 3000,

                    // Максимальное число неудачных циклов опроса — второй из двух критериев отключения.
                    // Работает вместе с device_timeout_ms по логике «И» (см. выше).
                    // Значение 0 — условие по числу циклов всегда выполнено,
                    // отключение определяется только по времени.
                    // Значение -1 — механизм отключения устройства выключен целиком
                    // (независимо от device_timeout_ms). Устройство всегда
                    // считается подключённым.
                    ...
                    // Интервал публикации неизменившихся значений каналов устройства (с)
                    // По истечении интервала значение канала публикуется, даже если оно не изменилось.
                    // В отличие от max_unchanged_interval, публикация не привязана к чтению канала:
                    // публикуется последнее прочитанное значение.
                    // Значение 0 (по умолчанию) отключает публикацию неизменившихся значений.
                    // Положительные значения меньше 5 секунд автоматически поднимаются драйвером до 5 секунд.
                    // Значение, заданное для канала, имеет больший приоритет.
                    ...
                    "channels": [
                        {
                            ...
                            // Тип регистра для записи, только для Modbus (задается при необходимости).
                            // Если задан, то для чтения используется тип регистра из "reg_type", а для записи — из "write_reg_type".
                            // Наличие "write_reg_type" делает канал доступным для записи, опция "readonly" при этом игнорируется.
                            // Нельзя использовать вместе с адресом с указанием битов ("address": "reg:shift:width").
                            "write_reg_type": "holding",
                            ...
                            //   "u24" - беззнаковое 24-битное целое (занимает 2 регистра, начиная с указанного)
                            //   "s24" - знаковое 24-битное целое (занимает 2 регистра, начиная с указанного)
                            ...
                            // Интервал публикации неизменившегося значения канала (с)
                            // Переопределяет значение, заданное для устройства.
                            // Значение 0 отключает публикацию неизменившихся значений канала.
                            // Положительные значения меньше 5 секунд автоматически поднимаются драйвером до 5 секунд.
                            "max_publish_interval": 60,
                            ...
                            // Для дискретных каналов указывайте "sporadic": канал будет обновляться только по событиям, без регулярного опроса.
                            // Для аналоговых каналов указывайте "semi-sporadic": события используются вместе с опросом — это позволяет избежать «застывания» значения между событиями.
                            "sporadic": true,
                            ...
                            // Специальное значение регистра, означающее ошибку.
                            // Если устройство вернуло это значение, драйвер считает канал находящимся в состоянии ошибки.
                            "error_value": "0xAA",
...
Если необходимо внести обратно-несовместимые изменения в шаблон без выпуска новой версии прошивки, действуйте по [WB-STD-003 §10.4](https://github.com/wirenboard/wb-standards):

1. **Переименуйте** текущий файл шаблона, добавив к имени постфикс `-deprecated`. Значение `device_type` внутри менять нельзя — оно сохранено в конфигурациях пользователей, и устаревший шаблон должен продолжать у них работать.

2. **Отметьте старый шаблон как устаревший**, чтобы скрыть его при добавлении новых устройств: добавьте `"deprecated": true` на верхний уровень JSON.

3. **Создайте новый шаблон**, добавив суффикс `-tpl1` (`-tpl2` и т.д.) и в имя файла, и в `device_type`: имя файла должно соответствовать `device_type` (WB-STD-003 §4.4).

   В итоге у вас получится два разных файла, например:

   ```files
   templates/config-wb-mr6c-v3-deprecated.json.jinja   (старый шаблон: "device_type": "wb-mr6c-v3", "deprecated": true)
   templates/config-wb-mr6c-v3-tpl1.json.jinja         (новый шаблон: "device_type": "wb-mr6c-v3-tpl1")
   ```

> **Обратите внимание:** в репозитории есть шаблоны, устаревшие по прежней схеме — с префиксом `tpl1_`/`tpl2_` в `device_type` нового шаблона и без суффикса в имени файла. Это legacy, переделывать их не нужно; новые замены шаблонов выполняются по схеме выше.
...
```jsonc
{
    ...
    // Устаревшее поле. Название типа устройства, которое будет отображаться в веб-конфигураторе.
    // В новых шаблонах не задаётся: название берётся из секции "translations" по ключу device_type,
    // см. раздел "Перевод названий параметров устройств".
    "title": "Device",
    ...
    "device": {
        ...
        "parameters": [
            {
                // Имя параметра, которое будет использовано в конфигурационном файле
                "id": "param1",

                // Устаревшее поле. Название параметра в веб-конфигураторе.
                // В новых шаблонах не задаётся: название берётся из секции "translations" по ключу "id"
                "title": "s22",
                ...
                // Тип регистра для записи, только для Modbus (задается при необходимости).
                // Если задан, то для чтения используется тип регистра из "reg_type", а для записи — из "write_reg_type".
                // Наличие "write_reg_type" делает параметр доступным для записи, опция "readonly" при этом игнорируется.
                // Нельзя использовать вместе с адресом с указанием битов ("address": "reg:shift:width").
                "write_reg_type": "holding",
                ...
                // Исключение: если новая прошивка только расширяет список "enum", параметр объявляется ещё раз
                // с тем же "id", "fw" новой прошивки и расширенным "enum".
                "fw": "1.2.3",
        ...
        // и не влияют на структуру конфигурационного файла
        "groups": [
            // Описание группы
            {
                // Устаревшее поле. Название группы, отображается в веб-конфигураторе.
                // В новых шаблонах не задаётся: название берётся из секции "translations" по ключу "id"
                "title": "Group 1",
...
У параметра, который упомянут в условии `condition`, может быть несколько объявлений с одним `id`. Например, когда список значений параметра зависит от режима устройства. Все такие объявления должны читать регистр одинаково. Поля `reg_type`, `format`, `scale`, `offset`, `round_to`, `word_order` и `byte_order` обязаны совпадать. Остальные поля, в том числе `enum`, могут различаться. Начиная с версии 2.265.0 драйвер проверяет это при первом использовании шаблона.

Зачем это нужно. Чтобы вычислить условие, драйвер читает значение параметра из устройства. Но какое из объявлений параметра действует, определяется другими условиями, и в момент чтения это ещё не известно. Поэтому если объявления преобразуют регистр по-разному, прочитанное значение будет зависеть от порядка вычислений. Когда поля чтения совпадают, по любому объявлению получается одно и то же значение, и драйверу не нужно рекурсивно разбирать зависимости между условиями.

### Перевод названий параметров устройств

Переводы используются в веб-конфигураторе для формирования интерфейса настройки устройств. Они описываются в шаблоне устройства в секции `translations`, которая содержит набор `"Язык": "Ключ": "Перевод"`.

Ключом перевода служит идентификатор объекта:

| Объект | Ключ перевода |
| --- | --- |
| шаблон устройства | `device_type` |
| группа | `id` группы |
| параметр | `id` параметра |
| канал | `name` канала |
| значения из `enum_titles`, `description` и прочие строки | сама строка на английском |

Английское название задаётся переводом на язык `en`, поэтому поле `title` у шаблона, групп и параметров задавать не нужно.

Переводы на язык `en` используются и как значение по умолчанию: если перевода на нужный язык нет, берётся английский, а если нет и его — идентификатор объекта.

Для совместимости со старыми шаблонами `title` остаётся ключом перевода, если он задан в шаблоне. Поэтому шаблоны, в которых ключом перевода служит английский текст из `title`, продолжают работать без изменений.
...
Драйвер ведёт два независимых уровня отслеживания обрыва связи: **уровень устройства** (переводит конкретное устройство в состояние «отключено») и **уровень порта/соединения** (сбрасывает и переоткрывает порт или TCP-сессию). На каждом уровне используется пара параметров, действующих по логике «И».

#### Уровень устройства: `device_timeout_ms` и `device_max_fail_cycles`

Устройство переводится драйвером во внутреннее состояние `DISCONNECTED`, когда одновременно выполнены оба условия:

1. с момента последнего успешного обмена с устройством прошло не менее `device_timeout_ms` миллисекунд;
2. подряд набрано не менее `device_max_fail_cycles` циклов опроса, в каждом из которых не был успешно прочитан ни один регистр устройства.

После перевода в `DISCONNECTED` устройство опрашивается в ограниченном режиме: если у устройства есть setup-секция — драйвер пробует её записать, иначе выполняется одна проверочная транзакция. Если первая попытка в цикле закончилась ошибкой — дальнейшие попытки в этом цикле не делаются, а пауза до следующей попытки постепенно растёт (шаг 500 мс, потолок 10 с). Первый успешный запрос расценивается как переподключение: setup-секция отправляется заново, состояние возвращается в `CONNECTED`.

Граничные значения:
- `device_timeout_ms: 0` — условие по времени всегда выполнено, отключение определяется только числом неуспешных циклов;
- `device_max_fail_cycles: 0` — условие по числу циклов всегда выполнено, отключение определяется только по времени;
- `device_timeout_ms: -1` или `device_max_fail_cycles: -1` — механизм отключения устройства полностью выключен (достаточно задать -1 одному из двух параметров); устройство всегда считается подключённым.

#### Уровень порта/соединения: `connection_timeout_ms` и `connection_max_fail_cycles`

Соединение сбрасывается (для TCP — закрытие сессии и переподключение; для последовательного порта — переинициализация) при одновременном выполнении двух условий:

1. с момента последнего успешного обмена по порту прошло не менее `connection_timeout_ms` миллисекунд;
2. подряд набрано не менее `connection_max_fail_cycles` циклов, ни в одном из которых по порту не было успешных обменов (практически это означает, что все устройства на порту уже переведены в `DISCONNECTED`).

Граничные значения:
- `connection_timeout_ms: 0` — условие по времени всегда выполнено, разрыв определяется только числом неуспешных циклов;
- `connection_max_fail_cycles: 0` — условие по числу циклов всегда выполнено, разрыв определяется только по времени;
- `connection_timeout_ms: -1` или `connection_max_fail_cycles: -1` — механизм переподключения полностью выключен (достаточно задать -1 одному из двух параметров).

#### Когда использовать какой критерий

Параметры допустимо использовать **и по отдельности, и вместе**. Комбинация по-прежнему работает по «И» — таймаут срабатывает, только когда истекло нужное время *и* пройдено нужное количество циклов.

- **Только `connection_timeout_ms`** (`connection_max_fail_cycles: 0`). Удобно, когда нужно реагировать строго по времени. Имейте в виду, что фактическое число попыток обращения к порту за это время зависит от других временных настроек (`poll_interval`, `guard_interval`, `response_timeout`); при их изменении может потребоваться подстроить и `connection_timeout_ms`.
- **Только `connection_max_fail_cycles`** (`connection_timeout_ms: 0`). Удобно, когда на порту много устройств и длительность цикла сильно варьируется в зависимости от числа отвечающих — гарантирует фиксированное минимальное число попыток до переподключения.
- **Оба параметра вместе (по умолчанию)**. Полезно, чтобы исключить срабатывание таймаута на случайных кратковременных ошибках: пока набирается счётчик циклов, одиночный сбой не приведёт к разрыву.
...
```mermaid
flowchart TD
    S(["Начало"]) --> P["Подождать poll_interval"]
    P --> Q1{"Есть регистры для опроса?"}
    Q1 -- "нет" --> P
    Q1 -- "да" --> Q2{"Устройство было отключено?"}
    Q2 -- "да" --> W["Для каждого регистра, ожидающего записи:<br/>подождать frame_timeout_ms,<br/>подождать guard_interval_us,<br/>записать регистр,<br/>дождаться подтверждения записи<br/>в течение response_timeout_ms"]
    W --> WF2["Подождать frame_timeout_ms<br/>по умолчанию 0 мс или задано протоколом"]
    Q2 -- "нет" --> WF2
    WF2 --> WG2["Подождать guard_interval_us<br/>по умолчанию 0 мкс"]
    WG2 --> READ["Послать запрос на чтение регистров"]
    READ --> RDATA["Ожидание данных регистров<br/>в течение response_timeout_ms<br/>по умолчанию 500 мс"]
    RDATA --> Q1

    classDef wait fill:#deb887,stroke:#8b6914,color:#000
    class P,W,WF2,WG2,RDATA wait
```

### Таймауты чтения фрейма: `frame_timeout_ms` и `force_frame_timeout`

Параметр `frame_timeout_ms` задаёт **один таймаут с двумя ролями** при чтении ответа от устройства:

1. **Максимальная пауза между байтами одного фрейма.** Если очередной байт не приходит в течение этого времени, драйвер считает, что устройство «замолчало», и прекращает приём.
2. **Детекция конца фрейма.** Когда драйвер не знает точной длины ответа или включён `force_frame_timeout`, окончание фрейма определяется по паузе: как только байты перестают приходить в течение `frame_timeout_ms`, фрейм считается принятым.

Фактически используемый таймаут рассчитывается так:

```
effective_frame_timeout = max(frame_timeout_ms, ceil(3.5 × время_передачи_байта))
```

где `время_передачи_байта` зависит от текущей скорости порта (например, при 9600 бит/с 8N1 ≈ 1.04 мс, значит минимум ≈ 3.7 мс). Коэффициент 3.5 соответствует стандарту Modbus RTU для межкадровой паузы (`STANDARD_FRAME_TIMEOUT_BYTES` в `src/modbus_base.h`; реализация — `TModbusDevice::GetFrameTimeout`). Отсюда два следствия:

- **Значение `0` не отключает механизм**: при `frame_timeout_ms: 0` фактически используется `ceil(3.5 × время_передачи_байта)`. В шаблонах WB-устройств часто ставят `0` именно для того, чтобы полагаться на автоматический расчёт от скорости, а не на статический дефолт 20 мс.
- Увеличивать `frame_timeout_ms` имеет смысл только для устройств сторонних производителей, которые вносят бóльшие паузы между байтами или отвечают с нестандартной задержкой после тишины.

#### Соответствие стандарту Modbus RTU

Стандарт Modbus RTU задаёт две характерные паузы: **1.5 символа** между байтами (inter-character) и **3.5 символа** между кадрами (inter-frame). У wb-mqtt-serial **одно** значение (`3.5 × время-символа`) используется для обеих ролей — отдельного 1.5-символьного межсимвольного таймаута у драйвера нет. В роли мастера это безопасно: драйвер просто даёт слейву больше времени на следующий байт, чем требует стандарт.

Рекомендация стандарта использовать на скоростях **выше 19200 бит/с** фиксированные значения 750 мкс (inter-character) и 1.75 мс (inter-frame) в wb-mqtt-serial **не применяется**: драйвер всегда считает пропорционально от текущего baud rate. На высоких скоростях эффективный таймаут поэтому получается меньше рекомендованных 1.75 мс. Со стандартными Modbus-устройствами это работает; для ускоренного опроса сторонних слейвов, жёстко придерживающихся 1.75 мс, может потребоваться поднять `frame_timeout_ms` вручную.

#### Флаг `force_frame_timeout`

По умолчанию (`force_frame_timeout: false`) драйвер полагается на знание структуры Modbus-ответа: как только получено ожидаемое число байт, приём завершается сразу, без ожидания тишины. Это ускоряет опрос.

При `force_frame_timeout: true` драйвер **всегда дожидается тишины** длиной `effective_frame_timeout` после последнего принятого байта, прежде чем считать фрейм завершённым. Всё, что пришло сверх ожидаемой длины, отбрасывается. Это строгое соответствие Modbus RTU, полезное когда слейв может досылать «хвост» (например, лишние байты от предыдущего ответа на плохой линии связи). Цена — некоторое снижение частоты опроса.

### Объединенное чтение регистров и его авто-отключение

Для ускорения опроса драйвер объединяет чтение соседних регистров в один запрос (см. `max_reg_hole`, `max_bit_hole`). Однако чтение так называемых «пустых» регистров — промежуточных адресов, не описанных в конфигурации, на некоторых устройствах приводит к ошибкам. Если при объединённом чтении, в которое попал хотя бы один «пустой» регистр, устройство возвращает ошибку, типичную для обращения к несуществующему регистру (для Modbus это `ILLEGAL_DATA_ADDRESS` или `ILLEGAL_DATA_VALUE`), драйвер перестаёт объединять эти регистры в пачку и читает их раздельно.
...
### Список портов и опрашиваемых устройств

Начиная с версии 2.271.0 список портов и опрашиваемых на них устройств можно получить одним MQTT RPC запросом. Ответ описывает конфигурацию, с которой запущен опрос, обмена с устройствами не происходит, поэтому ответ приходит сразу.

Для выполнения запроса необходимо отправить в топик `/rpc/v1/wb-mqtt-serial/ports/List/client_id`, где `client_id` это произвольное имя клиента, посылающего запрос, сообщение типа JSON. Метод не принимает параметров.

В качестве ответа в топике `/rpc/v1/wb-mqtt-serial/ports/List/client_id/reply` будет опубликована структура с данными. Ниже пример ответа для конфигурации с последовательным портом, портом Serial over TCP и портом Modbus TCP:

```jsonc
{
    "ports": [

        // Последовательный порт. Настройки связи передаются полностью,
        // даже если в конфигурации заданы не все,
        // значения по умолчанию уже подставлены
        {
            "path": "/dev/ttyRS485-1",
            "baud_rate": 9600,
            "data_bits": 8,
            ...
            // Устройства, опрашиваемые на порту
            "devices": [
                {
                    // Идентификатор устройства, из него составлены его топики в MQTT
                    "device_id": "wb-mr6cv3_25",

                    // Адрес устройства на шине в том виде, как задан в конфигурации,
                    // включая шестнадцатеричную запись.
                    // Поля нет у устройства без адреса, например настроенного
                    // для работы в широковещательном режиме
                    "slave_id": "25",

                    // Тип устройства, то есть шаблон, по которому оно настроено.
                    // Поля нет у устройства, настроенного без шаблона
                    "device_type": "WB-MR6C v.3",

                    // Протокол обмена с устройством, по умолчанию modbus.
                    // У устройства на порту Modbus TCP здесь тоже modbus,
                    // потому что транспорт описывает поле mode у порта
                    "protocol": "modbus"
                },
                ...
            ]
        ...
        {
            // Адрес из конфигурации, это может быть и имя хоста
            "ip": "192.168.1.10",
...
Порты и устройства, отключённые в конфигурации полем `"enabled": false`, не загружаются и в ответ не попадают, а устройство, опрос которого остановлен запросом `device/SetPoll`, и устройство, потерявшее связь, в ответе есть. Если конфигурация не загрузилась, метод возвращает пустой массив `ports`, причина неудачной загрузки видна в журнале сервиса.
    ...
    "items":[
        {
            ...
            // Серийный номер устройства, необязательный
            "sn": 4291508789,
...
```
Если у элемента задан `sn`, обмен идёт Быстрым Модбасом с адресацией по серийному номеру, а не по `slave_id`. Так можно настроить устройство, текущий адрес которого неизвестен или совпадает с адресом другого устройства на шине.

Поле `cfg.slave_id` меняет адрес устройства. Драйвер только записывает регистры и не проверяет, что устройство отозвалось с новыми настройками. Файл конфигурации драйвера при этом не меняется, поэтому после смены адреса или параметров связи устройство выпадет из опроса, пока его настройки не поправят в конфигурации.
...
Для выполнения запроса необходимо отправить в топик `wb-mqtt-serial/device/Probe/client_id`, где `client_id` - произвольное имя клиента, посылающего запрос, сообщение типа JSON со следующими параметрами:
```json
{
...
|`force`           |опциональный |`false`               |если параметр равен `true`, то настройки устройства будут считаны принудительно, даже если они уже есть в кэше
...
|`parameters`|JSON-объект c актуальными настройками устройства (может быть пустым в случае, если никакие настройки не описаны в шаблоне или не были считаны, например, из-за несоответствия версии устройства и описания настроек в шаблоне)|
...
В случае, если опрашиваемое устройство присутствует в конфигурационном файле `/etc/wb-mqtt-serial.conf`, результат опроса кэшируется, а при повторных запросах для ответа используются данные из кэша, если в запросе не был указан параметр `"force": true`. Следует иметь в виду, что изменение настроек устройства посредством RPC запроса `wb-mqtt-serial/port/Load` с использованием "сырых" данных не влияет на содержимое кэша и может привести к несоответствию реальных настроек и настроек, полученных из кэша.

В случае потери связи с устройством его данные автоматически удаляются из кэша.
...
|`slave_id`        |обязательный |                      |адрес устройства
|`device_type`     |обязательный |                      |тип устройства, как указано в соответствующем шаблоне
|`channels`        |опциональный |                      |JSON-массив с названиями каналов, которые необходимо считать, например, `["Input 1", "Input 2", "Input 3"]`, если отсутствует в запросе или является пустым массивом, будут считаны все каналы
...
|`readonly`|JSON-массив cо списком названий каналов, которые можно считывать, но нельзя записывать|

Начиная с версии 2.268.0 значение `"unsupported"` в данных ответа означает, что собственный регистр пункта не был прочитан: устройство не поддерживает регистр или считанное значение совпало с маркером `error_value` или `unsupported_value` из шаблона.
...
Условия `condition` объявлений каналов и параметров вычисляются по значениям параметров из запроса, недостающие значения параметров условий считываются из устройства.
...
|`parameters`      |опциональный |                      |JSON-объект с данными параметров, которые необходимо записать, например, `{"in1_mode": 0, "in2_mode": 1, "in3_mode": 2}`. Значения должны быть числами, как в конфигурации устройства
|`response_timeout`|опциональный |500 ms                |таймаут чтения первого байта в миллисекундах. Значение сравнивается с параметром `response_timeout_ms` соответствующего порта в настройках wb-mqtt-serial и используется максимальное
|`frame_timeout`   |опциональный |20 ms                 |таймаут чтения каждого последующего байта в миллисекундах
|`total_timeout`   |опциональный |10000 ms              |таймаут выполнения RPC-запроса в миллисекундах
|`modbus_mode`     |опциональный |`RTU`                 |протокол работы с Modbus устройствами при использовании TCP порта (с указанием `ip` и `port` в запросе), возможные значения: `RTU` - для работы через прозрачные шлюзы Modbus RTU-over-TCP, `TCP` - для работы по протоколу Modbus TCP.
...
После остановки опрос будет автоматически возобновлен через 10 минут. Повторные запросы на остановку опроса продлевают это время (автоматическое возобновление опроса происходит через 10 минут после последнего запроса на остановку).
...
Начиная с версии 2.232.0 прошивки устройств Wiren Board, их загрузчики и прошивки компонентов обновляет wb-mqtt-serial. Раньше эти запросы обслуживал сервис wb-device-manager, теперь логика обновления живёт в драйвере, который работает с устройством напрямую. По своей инициативе драйвер прошивки не меняет, обновление выполняется по запросу.

Файлы прошивок и загрузчиков берутся с сервера `https://fw-releases.wirenboard.com`. Канал обновлений выбирается по значению `SUITE` из файла `/usr/lib/wb-release`, поэтому на контроллере с тестовым каналом предлагаются тестовые прошивки. Ответы сервера кешируются в памяти сервиса: список прошивок на 10 минут, список загрузчиков на 30 минут, скачанные файлы прошивок на 2 часа. Неудачная попытка получить списки версий с сервера повторяется не раньше, чем через минуту.

#### Запрос информации о прошивке

Для выполнения запроса необходимо отправить в топик `/rpc/v1/wb-mqtt-serial/fw-update/GetFirmwareInfo/client_id`, где `client_id` - произвольное имя клиента, посылающего запрос, сообщение типа JSON со следующими параметрами:

|Параметр   |Тип          |Значение по умолчанию |Описание |
|-----------|-------------|----------------------|---------|
|`slave_id` |обязательный |                      |адрес устройства, целое число от 0 до 255
|`port`     |обязательный |                      |настройки порта, поля приведены в таблице ниже
|`protocol` |опциональный |`modbus`              |протокол обмена с устройством. При указании значения `modbus` в порт отправляется Modbus RTU сообщение, для работы по Modbus TCP укажите `modbus-tcp`

Поля объекта `port`:

|Поле        |Тип          |Значение по умолчанию |Описание |
|------------|-------------|----------------------|---------|
|`path`      |обязательный |                      |путь к последовательному порту, например `/dev/ttyRS485-1`. Для TCP-порта из конфигурации wb-mqtt-serial указывается его описание в виде `адрес:порт`, например `192.168.1.10:23`
|`baud_rate` |опциональный |9600                  |скорость порта
|`parity`    |опциональный |`N`                   |чётность, `N`, `O` или `E`
|`data_bits` |опциональный |8                     |количество бит данных
|`stop_bits` |опциональный |1                     |количество стоп-бит
...
Драйвер читает регистры устройства и сравнивает установленные версии с версиями на сервере обновлений. Если для устройства уже выполняется обновление, запрос отклоняется с ошибкой.

В качестве ответа в топике `/rpc/v1/wb-mqtt-serial/fw-update/GetFirmwareInfo/client_id/reply` будет опубликована структура с данными:
...
```jsonc
{
    ...
    // Признак того, что сервер обновлений предлагает для устройства
    // более новую прошивку или загрузчик
    "can_update": false,

    // Признак того, что доступная прошивка новее установленной
    "fw_has_update": false,

    // Признак того, что доступный загрузчик новее установленного
    "bootloader_has_update": false,

    // Модель устройства, прочитана из регистров 200-219,
    // а если они недоступны, то из регистров 200-205
    "model": "WB-LED",
    ...
    "components": {
        ...
        "0": {
            ...
            // Модель компонента
            "model": "CO2 Sensor NS8",

            // Установленная прошивка компонента.
            // Если пустая строка, то обновление компонента завершилось неудачно
            // и в данный момент на компоненте отсутствует прошивка
            "fw": "MF1.01D",

            // Прошивка компонента, доступная для обновления.
            // Если пустая строка, то не удалось получить информацию о новых прошивках
            "available_fw": "MF1.03D",

            // Признак того, что доступная прошивка компонента
...
Доступные версии берутся из кеша ответов сервера обновлений, сам запрос в интернет не ходит. Если получить данные с сервера не удалось, например у контроллера нет доступа в интернет, поля `available_fw` и `available_bootloader` пустые, а `can_update` равен `false`. Так же пустыми они остаются для устройств, которые не поддерживают обновление прошивки по Modbus, например для WB-MSW LORA.

Если сигнатура прошивки, версия прошивки или версия загрузчика прочитаны с ошибкой или содержат непечатные символы, соответствующие поля ответа пустые. Это штатная ситуация для устройств сторонних производителей, которые отвечают на чтение регистров Wiren Board.

### Служебные RPC

Кроме описанных выше запросов драйвер обслуживает служебные RPC. Их используют веб-интерфейс и служебные сервисы Wiren Board, эти запросы могут меняться без сохранения обратной совместимости, поэтому для интеграций они не предназначены. Описаны они в отдельном документе, [docs/private_rpc.md](docs/private_rpc.md).
...
Для всех регистров сервис ведёт внутреннюю базу последних значений и публикует их при старте. База хранится в оперативной памяти контроллера и периодически сбрасывается на диск (раз в минуту, а также при корректной остановке сервиса). Поэтому, если значения не успели сброситься на диск до перезапуска, после старта сервис сначала опубликует последнее сохранённое значение, а потом — актуальное, полученное чтением из регистра. Чтобы не потерять информацию о нажатиях и не публиковать устаревшие значения, для регистров с типом `press_counter` база сбрасывается на диск сразу после обновления значения.

Счётчики протокола GTD IOT (`single_press_counter`, `long_press_counter`) ведёт сам сервис, а не устройство, см. раздел [GTD IOT](#gtd-iot). Из описанного выше к ним относятся два правила: 0, в который счётчики сбрасываются при переподключении панели, не публикуется, и база сбрасывается на диск сразу после обновления значения. События "Быстрого Модбаса" для этого не нужны: переподключение панели драйвер определяет по неудачным опросам.

Между перезапусками сервиса эти счётчики не сохраняются, но 0, с которого они начинаются, тоже не публикуется: до первого нажатия у регистра счётчика нет значения. Поэтому после перезапуска сервиса (например, после сохранения конфигурации в веб-интерфейсе) контрол сохраняет значение, опубликованное до перезапуска, а первое нажатие публикует 1. Так же ведёт себя счётчик устройства Wiren Board после перезагрузки устройства: последовательность значений счётчика не монотонна, правила не должны рассчитывать на то, что новое значение больше предыдущего. На новой установке, где сохранённого значения нет, контрол создаётся со значением 0 и остаётся нулевым до первого нажатия. Пока у счётчика нет значения, драйвер не выставляет на его контроле и ошибку чтения: снятие ошибки публикуется только вместе со значением, поэтому ошибка осталась бы на контроле до первого нажатия. О недоступности панели сообщают `meta/error` устройства и остальные его контролы.
...
Во фреймах разных протоколов первый байт несёт разную информацию:

- Uniel — всегда 0xff;
- ИВТМ — всегда 0x24 (`$`);
- Modbus, Меркурий 230, Милур — идентификатор slave-устройства.

При совмещении таких протоколов на одной шине нужно следить, чтобы slave_id у Modbus/Меркурий/Милур не совпадал с префиксами фреймов других протоколов. Например, у Милура по умолчанию slave_id равен 0xff — это совпадает с префиксом фреймов Uniel и приводит к конфликту.
...
При любых настройках порта фактический обмен начинается в режиме 300 7E1, в процессе опроса переключается на 9600 7E1.
...
Начиная с версии 2.279.0 драйвер поддерживает кнопочные панели GTD IOT серии 24A/24X. Запросы - Modbus RTU (функции 0x03 и 0x06), но ответы на чтение нестандартные, поэтому опрашивать панели как Modbus-устройства нельзя. Поддерживается только режим опроса. Кадры, которые панель отправляет сама в активном режиме, драйвер не принимает, поэтому панель должна быть в режиме опроса: в регистре режима `0x1003` бит 2 должен быть сброшен. В шаблоне это делает секция `setup`.

Панели выпускаются на 1-6 кнопок, для каждого числа кнопок есть свой шаблон: от `gtd-iot-24x-1` до `gtd-iot-24x-6`. Выбирайте шаблон по числу кнопок панели: от него зависит, сколько статусов читает драйвер.

Типы регистров:

- `holding` - регистр панели, адрес - адрес регистра. Поддерживаются битовые поля (`"0x1008:1:1"`), запись в них выполняется как чтение, изменение и запись регистра. Каналы с одним адресом читаются одним запросом.
- `status` - состояние кнопки: 0 - отпущена, 1 - нажата, 2 - удерживается дольше ~2 с.
- `single_press_counter` - счетчик одиночных нажатий: нажатие закончилось, не став длинным.
- `long_press_counter` - счетчик длинных нажатий: кнопку удерживали дольше ~2 с.

Для кнопок адрес - регистр статуса кнопки: `0x1310` для K1, `0x1311` для K2 и так далее до `0x1317`.

Все каналы кнопок устройства читаются вместе, двумя запросами: код нажатия `0x100B`, затем статусы кнопок - от K1 до кнопки с наибольшим настроенным адресом. Нажатия кнопок, которых нет в конфигурации, драйвер отбрасывает. Регистр статуса показывает нажатие, только пока кнопка нажата, а его чтение сбрасывает код нажатия. Поэтому код читается первым, а нажатие, начавшееся между запросами, видно по статусу: кнопка в этот момент еще нажата. Нажатие, которое закончилось между опросами, канал состояния показывает как 1 на один опрос. Счетчики ведет драйвер, а не панель: после перезапуска сервиса отсчет начинается заново, но 0 не публикуется, контрол сохраняет прежнее значение до первого нажатия, см. [Обработка счётчиков нажатий](#обработка-счётчиков-нажатий).

Кнопки нужно опрашивать чаще, чем раз в 2 секунды: панель сама сбрасывает код нажатия примерно через 2 с, а удержание, целиком уместившееся между опросами, будет засчитано как одиночное нажатие. Поэтому не задавайте каналам кнопок `read_period_ms` и `read_rate_limit_ms`: кроме замедления, разные периоды разводят каналы кнопок по разным опросам, и короткое нажатие может не попасть в канал состояния.

Не опрашивайте `0x100B` и регистры статусов отдельными каналами `holding`: такие чтения сбрасывают код нажатия.

Ограничения панели, которые драйвер обойти не может:

- код нажатия хранит только последнюю нажатую кнопку. Если между двумя опросами нажали и отпустили одну кнопку, а потом другую, первое нажатие теряется. Несколько нажатий одной кнопки между опросами считаются одним. Помогает только частый опрос, поэтому не увеличивайте `min_request_interval` у устройства;
- пока одна кнопка зажата, панель нажатия других кнопок не регистрирует.
...
|                     Device                     | device_type    | id_prefix      | name_prefix           |
|:----------------------------------------------:|:--------------:|:--------------:|:---------------------:|
| Кнопочная панель GTD IOT серии 24A/24X на 1 кнопку | gtd-iot-24x-1 | gtd_iot_24x   |  GTD IOT 24X          |
| Кнопочная панель GTD IOT серии 24A/24X на 2 кнопки | gtd-iot-24x-2 | gtd_iot_24x   |  GTD IOT 24X          |
| Кнопочная панель GTD IOT серии 24A/24X на 3 кнопки | gtd-iot-24x-3 | gtd_iot_24x   |  GTD IOT 24X          |
| Кнопочная панель GTD IOT серии 24A/24X на 4 кнопки | gtd-iot-24x-4 | gtd_iot_24x   |  GTD IOT 24X          |
| Кнопочная панель GTD IOT серии 24A/24X на 5 кнопок | gtd-iot-24x-5 | gtd_iot_24x   |  GTD IOT 24X          |
| Кнопочная панель GTD IOT серии 24A/24X на 6 кнопок | gtd-iot-24x-6 | gtd_iot_24x   |  GTD IOT 24X          |
