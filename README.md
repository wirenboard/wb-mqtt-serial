...
При необходимости внести обратно несовместимые изменения в шаблон, его необходимо отметить устаревшим (добавить `"deprecated": true`) и создать его копию (при этом в `device_type` добавить префикс с версией `tpl1_`, `tpl2_` и т.д.).
