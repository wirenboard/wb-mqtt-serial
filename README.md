...
Единичные запросы для опроса незаметны, но постоянное общение с устройствами через RPC отбирает время у штатного опроса: замедляется обновление значений, задерживаются события и нажатия, страдают все устройства на порту. Поэтому RPC — служебный канал к шине, а не способ реализовать драйвер устройства: он нужен, чтобы отправить в шину отдельную команду, не останавливая драйвер. Штатный путь для нового устройства или протокола — шаблон или реализация протокола внутри драйвера.

| Характер запросов | Примеры | Ограничения |
|---|---|---|
| Разовые и служебные операции | Настройка и прошивка устройств, диагностика, единичный запрос по действию пользователя или правила, сканирование шины | Нет |
| Редкие периодические запросы | Интеграция без требований к задержке; одно-два устройства на отдельном порту | Допустимость зависит от нагрузки на конкретной шине. Требуется согласование с командой Wiren Board |
| Постоянный опрос устройств | RPC как основной способ обмена с устройством | Не допускается; устройство добавляется в конфигурацию драйвера шаблоном или реализацией протокола |

Численного порога запросов нет: нагрузка зависит от времени выполнения запросов (скорость порта, длина посылок, таймауты) и от требований остальных устройств на шине.
...
Для упрощенного использования данного функционала написана [Python-библиотека](https://github.com/wirenboard/python-mqtt-rpc/). Также по [ссылке](https://github.com/wirenboard/modbus-utils-rpc) доступна утилита для работы с modbus-устройствами при помощи RPC-функционала wb-mqtt-serial.
Ограничения по частоте запросов те же, что для [прямого чтения и записи в порт](#прямое-чтение-и-запись-в-порт): RPC предназначен для разовых операций, а не для постоянного опроса устройств.
Для выполнения запроса необходимо отправить в топик `wb-mqtt-serial/port/Load/client_id`, где `client_id` - произвольное имя клиента, посылающего запрос, сообщение типа JSON со следующими параметрами:
