...
Для организации отправки команд управления можно использовать канал с типом регистра `command` или `command_with_param`.
`command` - команда без параметров.
`command_with_param` - команда с одним параметром размером 1 байт. Значение параметра берётся из числа, записанного в `/on`-топик.
