...
В состав пакета изначально входят шаблоны конфигурации некоторых поддерживаемых устройств. Они хранятся в папке `/usr/share/wb-mqtt-serial/templates`. Для хранения пользовательских шаблонов используется папка `/etc/wb-mqtt-serial.conf.d/templates`. 
...
```jsonc
{
    ...
    // Группа устройств в выпадающем списке в настройках последовательного порта в веб-интерфесе контроллера
...
Переводы используются в веб-конфигураторе для формирования интерфейса настройки устройств. Они описываются в шаблоне устройства в секции `translations`. Эта секция содержит переводы строк, которые могут встечаться в названиях парметров, отчетах об ошибках и т.д. Секция содержит набор параметров `"Язык": "Строка на английском": "Перевод"`.
Например:
```jsonc
{
    "device_type": "Example",
