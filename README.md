...
- GTD IOT (кнопочные панели серии 24X),
...
Всё описанное в этом разделе относится только к типу `press_counter`. Счётчики протокола GTD IOT (`single_press_counter`, `long_press_counter`) ведёт сам сервис, а не устройство, поэтому они не сохраняются между перезапусками сервиса и начинаются с 0, см. раздел [GTD IOT](#gtd-iot).
...
Все каналы кнопок устройства читаются вместе, двумя запросами: код нажатия `0x100B`, затем статусы всех кнопок. Регистр статуса показывает нажатие, только пока кнопка нажата, а его чтение сбрасывает код нажатия. Поэтому код читается первым, а нажатие, начавшееся между запросами, видно по статусу: кнопка в этот момент еще нажата. Нажатие, которое закончилось между опросами, канал состояния показывает как 1 на один опрос. Счетчики ведет драйвер, после его перезапуска они начинаются с 0.
...
|                     Device                     | device_type    | id_prefix      | name_prefix           |
|:----------------------------------------------:|:--------------:|:--------------:|:---------------------:|
| Кнопочная панель GTD IOT серии 24X на 1-6 кнопок | gtd-iot-24x  | gtd_iot_24x    |  GTD IOT 24X          |
