...
- [Особенности работы драйвера](#особенности-работы-драйвера)
  ...
  - [RPC: Список портов и опрашиваемых устройств](#rpc-список-портов-и-опрашиваемых-устройств)
  - [RPC: Прямое чтение и запись в порт](#rpc-прямое-чтение-и-запись-в-порт)
  - [RPC: Чтение и запись по протоколу Modbus](#rpc-чтение-и-запись-по-протоколу-modbus)
  - [RPC: Прямая установка параметров связи устройства](#rpc-прямая-установка-параметров-связи-устройства)
  - [RPC: Получение информации об устройстве Wiren Board](#rpc-получение-информации-об-устройстве-wiren-board)
  - [RPC: Чтение настроек устройств](#rpc-чтение-настроек-устройств)
  - [RPC: Чтение каналов и параметров устройств](#rpc-чтение-каналов-и-параметров-устройств)
  - [RPC: Запись каналов и параметров устройств](#rpc-запись-каналов-и-параметров-устройств)
  - [RPC: Управление опросом устройств](#rpc-управление-опросом-устройств)
  - [RPC: Обновление прошивок устройств Wiren Board](#rpc-обновление-прошивок-устройств-wiren-board)
    ...
    "ports": [
        {
            ...
            // Отключить Быстрый Модбас на порту: события, поиск и настройку устройств по серийному номеру.
            // По умолчанию - false.
            "disable_fast_modbus": false,
            ...
            "devices": [
                {
                    ...
                    // Отключить события Быстрого Модбаса для устройства, его каналы опрашиваются обычным чтением.
                    // По умолчанию - false.
                    "disable_fast_modbus": false,
...
<a name="список-портов-и-опрашиваемых-устройств"></a>

### RPC: Список портов и опрашиваемых устройств
...
<a name="прямое-чтение-и-запись-в-порт"></a>

### RPC: Прямое чтение и запись в порт

Существует возможность выполнить запись и чтение из порта посредством MQTT RPC запроса. Выполнение запроса встраивается в цикл опроса устройств таким образом, что *запрос выполнится с высоким приоритетом* сразу после окончания текущего цикла опроса, поэтому не допускается использовать RPC для постоянного опроса устройств (если требуется постоянный опрос, то для таких устройств создается шаблон или выполняется реализация протокола внутри wb-mqtt-serial).
Через RPC без ограничений выполняются разовые и служебные операции, допустимы редкие периодические запросы с учётом требований на конкретной шине.

Для упрощенного использования данного функционала написана [Python-библиотека](https://github.com/wirenboard/python-mqtt-rpc/) и доступна [утилита для работы с modbus-устройствами](https://github.com/wirenboard/modbus-utils-rpc) при помощи RPC-функционала wb-mqtt-serial.
...
<a name="чтение-и-запись-по-протоколу-modbus"></a>

### RPC: Чтение и запись по протоколу Modbus
...
Ограничения по частоте запросов те же, что для [прямого чтения и записи в порт](#rpc-прямое-чтение-и-запись-в-порт): RPC предназначен для разовых операций, а не для постоянного опроса устройств.
Для выполнения запроса необходимо отправить в топик `wb-mqtt-serial/port/Load/client_id`, где `client_id` - произвольное имя клиента, посылающего запрос, сообщение типа JSON со следующими параметрами:
...
<a name="прямая-установка-параметров-связи-устройства"></a>

### RPC: Прямая установка параметров связи устройства
...
```
Если у элемента задан `sn`, обмен идёт Быстрым Модбасом с адресацией по серийному номеру, а не по `slave_id`. Так можно настроить устройство, текущий адрес которого неизвестен или совпадает с адресом другого устройства на шине. Если на порту Быстрый Модбас недоступен (задан `disable_fast_modbus` или это порт Modbus TCP без `connected_to_mge`), запрос с элементами, у которых есть `sn`, отклоняется с ошибкой целиком.
...
<a name="получение-информации-об-устройстве-wiren-board"></a>

### RPC: Получение информации об устройстве Wiren Board
...
<a name="чтение-настроек-устройств"></a>

### RPC: Чтение настроек устройств
...
<a name="чтение-каналов-и-параметров-устройств"></a>

### RPC: Чтение каналов и параметров устройств
...
<a name="запись-каналов-и-параметров-устройств"></a>

### RPC: Запись каналов и параметров устройств
...
<a name="управление-опросом-устройств"></a>

### RPC: Управление опросом устройств
...
<a name="обновление-прошивок-устройств-wiren-board"></a>

### RPC: Обновление прошивок устройств Wiren Board
